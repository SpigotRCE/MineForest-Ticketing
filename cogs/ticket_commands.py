...
class Ticket_Command(commands.Cog):
    ...
    async def add(self, ctx, member: Option(discord.Member, description="Which Member you want to add to the Ticket",
        ...
        if "ticket-" in ctx.channel.name or "ticket-closed-" in ctx.channel.name:
            ...
            self.embed = discord.Embed(description=f'You can only use this command in a Ticket!',
    ...
    async def delete_ticket(self, ctx):
        ...
        transcript = await chat_exporter.export(ctx.channel, limit=200, tz_info=TIMEZONE, military_time=military_time,
                                                bot=self.bot)
